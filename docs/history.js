...
import { getCachedDay, putCachedDay } from "./dayCache.js";
 ...
 * IndexedDBに永続キャッシュするため、一度取得した日は次回以降ネットワーク不要。
 * @param {string} baseDate YYYYMMDD
 * @param {number} lookbackDays
 * @param {(done: number, total: number) => void} [onProgress]
 */
export async function fetchRecentDays(baseDate, lookbackDays, onProgress) {
  const dates = [];
  ...
  async function worker() {
    while (idx < dates.length) {
      ...
      let data = await getCachedDay(date);
      if (!data) {
        data = await fetchDayRaw(date);
        if (data) await putCachedDay(date, data);
      }
 ...
 */
export function buildRacerHistory(dayDatasets, registrationNumber) {
  const entries = [];
